...
class IdentifyEvent {
  ...
  constructor(config?: IdentifyEventConfig) {
    if (config !== undefined) this.config = { ...this.config, ...config };
  }
  ...
  onTouchMove = (pageX: number) => {
    ...
    this.lastFiveXTouchMoveIndex =
      this.lastFiveXTouchMoveIndex >= 4 ? 0 : this.lastFiveXTouchMoveIndex + 1;
  ...
  protected isSwipe = (pageY: number): boolean => {
    ...
    return (
      this.elapsedTime <= this.config.allowedTime &&
      Math.abs(this.dist) >= this.config.threshold &&
      Math.abs(pageY - this.startY) <= this.config.restraint
    );
  };
  ...
  protected isSwipeAfterMoving = (pageY: number): boolean => {
    ...
    return (
      Math.abs(this.lastFiveXToucheMoveSum) > 50 &&
      Math.abs(pageY - this.startY) <= this.config.restraint &&
      this.elapsedTimeAfterMove <= this.config.allowedTimeToMoveSwipe
    );
  };
